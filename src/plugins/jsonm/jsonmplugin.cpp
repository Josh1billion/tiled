...
bool JsonMapFormat::write(const Tiled::Map *map, const QString &fileName)
{
    ...
    QString output = writer.result();

    QRegularExpression re1(" +\"rotation\".+\n");
    QRegularExpression re2(" +\"id\".+\n");
    QRegularExpression re3(" +\"visible\".+\n");
    QRegularExpression re4(" +\"name\".+\n");
    output.replace(re1, "");
    output.replace(re2, "");
    output.replace(re3, "");
    output.replace(re4, "");
...
{
    if (mSubFormat == Json)
        return tr("Minimalist JSON map files (*.json)");
    else
